...
export function SettingsActionTile({
  ...
  return (
    <button
      type={type}
      className={cn(
        "flex min-h-20 w-full items-start gap-3 rounded-2xl border px-4 py-3 text-left transition-[background-color,border-color,color,box-shadow,transform] duration-200 ease-out hover:-translate-y-px focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-ring/15 disabled:pointer-events-none disabled:opacity-50",
        isPrimary
    ...
    >
      ...
      <span className="space-y-1">
        ...
        <span
          className={cn("block text-xs leading-[1.125rem]", isPrimary ? "text-background/75" : "text-muted-foreground")}
        >
          {description}
        </span>
      </span>
